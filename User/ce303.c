...
uint8_t start_buf[5] = {0x2F, 0x3F, 0x21, 0x0D, 0x0A};
...
void ce303_voltage (void) // функция чтения напряения из счетчика СЕ303
{
	uint16_t temp;
	uint16_t temp_mil;
	...
	HAL_UART_Transmit_DMA(&huart6, &start_buf[0], 5); // передаем команду
	HAL_Delay(1000); // ждем
...
void ce303_current (void) // функция чтения тока из счетчика СЕ303
{
	uint16_t temp;
	uint16_t temp_mil;
